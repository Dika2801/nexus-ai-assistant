...
export const ToolsView: React.FC<ToolsViewProps> = ({
  ...
  const [activeTool, setActiveTool] = useState<'image' | 'doc' | 'search' | 'voice' | 'faceswap' | 'deploy'>('image');
  ...
  // Tool 5: Face Swap State
  const [faceSourceImage, setFaceSourceImage] = useState<{ name: string; base64: string; mimeType: string } | null>(null);
  const [faceTargetImage, setFaceTargetImage] = useState<{ name: string; base64: string; mimeType: string } | null>(null);
  ...
  const [faceEnhanceStrength, setFaceEnhanceStrength] = useState(0.6);
  const faceSourceInputRef = useRef<HTMLInputElement>(null);
  const faceTargetInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleTestSTT = async () => {
    if (isRecordingTest) {
      // Sedang merekam -> stop & transkripsi via OpenAI Whisper
      setIsRecordingTest(false);
      setSttTestTranscript('Mentranskripsi...');
      try {
        const text = await VoiceManager.stopRecordingAndTranscribe(
          userId,
          (uid, base64, mimeType, lang) => ApiClient.transcribeAudio(uid, base64, mimeType, lang),
          (voiceSettings.language || 'id-ID').split('-')[0]
        );
        setSttTestTranscript(text || '(tidak ada suara terdeteksi)');
      } catch (err: any) {
        setSttTestTranscript(`Error: ${err.message}`);
      }
      return;
    }

    setSttTestTranscript('');
    if (VoiceManager.isRecordingSupported()) {
      const started = await VoiceManager.startRecording((err) => {
        setSttTestTranscript(`Error: ${err}`);
        setIsRecordingTest(false);
      });
      if (started) setIsRecordingTest(true);
      return;
    }

    // Fallback ke Web Speech API browser jika MediaRecorder tidak didukung
  ...
  // Face Swap Handlers
  const handleFaceImageUpload = (which: 'source' | 'target') => (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (event) => {
      const base64 = event.target?.result as string;
      const payload = { name: file.name, base64, mimeType: file.type || 'image/jpeg' };
      if (which === 'source') setFaceSourceImage(payload);
      else setFaceTargetImage(payload);
  ...
  const handleRunFaceSwap = async () => {
    if (!faceSourceImage || !faceTargetImage) return;
    setIsFaceSwapping(true);
    setFaceSwapResult(null);
    setFaceSwapError(null);

    try {
      const res = await ApiClient.faceSwap(userId, {
        sourceImageBase64: faceSourceImage.base64,
        targetImageBase64: faceTargetImage.base64,
        enhanceStrength: faceEnhanceStrength,
      });
      setFaceSwapResult(res.imageUrl);
    } catch (err: any) {
      setFaceSwapError(err.message || 'Gagal memproses face swap.');
    } finally {
  ...
  const toolTabs = [
    ...
    { id: 'faceswap' as const, label: 'Face Swap', icon: Users },
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-3 py-4 space-y-4 pb-20">
      ...
      {activeTool === 'faceswap' && (
        <div className="space-y-4 bg-[#111111] border border-white/5 rounded-2xl p-4 sm:p-5">
          ...
          <p className="text-[11px] text-white/50 leading-relaxed">
            Gunakan hanya untuk foto yang kamu miliki hak/izinnya, atau dengan persetujuan eksplisit dari orang
            yang wajahnya digunakan. Hasil otomatis diberi watermark "AI Generated".
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <input
                ...
                type="file"
                accept="image/*"
                onChange={handleFaceImageUpload('source')}
                className="hidden"
              />
              <p className="text-[11px] font-semibold text-white/70 mb-1.5">Foto Sumber (wajah yang dipakai)</p>
              {!faceSourceImage ? (
                <button
                  onClick={() => faceSourceInputRef.current?.click()}
                  className="w-full border border-dashed border-white/15 hover:border-blue-500/50 rounded-xl p-6 flex flex-col items-center justify-center text-center group transition-colors bg-[#080808]"
                >
                  <Upload className="w-5 h-5 text-white/40 group-hover:text-blue-400 mb-1.5" />
                  <p className="text-[11px] text-white/50">Pilih Foto Wajah</p>
                </button>
              ) : (
                <div className="relative bg-[#080808] p-2 rounded-xl border border-white/5">
                  <img src={faceSourceImage.base64} alt="Sumber" className="w-full h-40 object-cover rounded-lg" />
                  <button
                    onClick={() => faceSourceInputRef.current?.click()}
                    className="mt-1.5 text-[11px] text-white/50 hover:text-white underline w-full text-center"
                  >
                    Ganti Foto
                  </button>
                </div>
            ...
            <div>
              <input
                ref={faceTargetInputRef}
                type="file"
                accept="image/*"
                onChange={handleFaceImageUpload('target')}
                className="hidden"
              />
              <p className="text-[11px] font-semibold text-white/70 mb-1.5">Foto Target (wajah diganti)</p>
              {!faceTargetImage ? (
                <button
                  onClick={() => faceTargetInputRef.current?.click()}
                  className="w-full border border-dashed border-white/15 hover:border-blue-500/50 rounded-xl p-6 flex flex-col items-center justify-center text-center group transition-colors bg-[#080808]"
                >
                  <Upload className="w-5 h-5 text-white/40 group-hover:text-blue-400 mb-1.5" />
                  <p className="text-[11px] text-white/50">Pilih Foto Target</p>
                </button>
              ) : (
                <div className="relative bg-[#080808] p-2 rounded-xl border border-white/5">
                  <img src={faceTargetImage.base64} alt="Target" className="w-full h-40 object-cover rounded-lg" />
                  <button
                    onClick={() => faceTargetInputRef.current?.click()}
                    className="mt-1.5 text-[11px] text-white/50 hover:text-white underline w-full text-center"
                  >
          ...
          <div>
            <label className="block text-[11px] font-semibold text-white/70 mb-1">
              Kekuatan Enhance/Restoration ({faceEnhanceStrength.toFixed(2)})
            </label>
            <input
              type="range"
              min="0"
          ...
          <button
            id="btn-run-face-swap"
            onClick={handleRunFaceSwap}
            disabled={!faceSourceImage || !faceTargetImage || isFaceSwapping}
            className="w-full py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white transition-all"
          >
            {isFaceSwapping ? (
              <>
                <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                <span>Memproses Face Swap...</span>
              </>
            ) : (
              <>
                <Sparkles className="w-3.5 h-3.5" />
                <span>Proses Face Swap</span>
              </>
            )}
          </button>

          {faceSwapError && (
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-3 text-xs text-rose-300 leading-relaxed">
              {faceSwapError}
            </div>
          )}

          {faceSwapResult && (
            <div className="bg-[#080808] border border-white/5 rounded-xl p-3 space-y-2">
              <p className="text-[11px] font-semibold text-white/70">Hasil Face Swap</p>
              <img src={faceSwapResult} alt="Hasil Face Swap" className="w-full rounded-lg border border-white/10" />
            </div>
          )}
        </div>
      )}
      ...
      {/* ---------------------------------------------------- */}
      {/* TOOL 6: APK & ZERO COST DEPLOY HUB */}
      {/* ---------------------------------------------------- */}
      {activeTool === 'deploy' && (
        <div className="space-y-4 bg-[#111111] border border-white/5 rounded-2xl p-4 sm:p-5">
          <div className="flex items-center justify-between">
